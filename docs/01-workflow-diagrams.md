...
5.  **FINAL_SUBMITTED**: Adviser confirms all revisions are complied after defense approval.
6.  **ARCHIVED**: Project is completed and archived by the Coordinator.
...
stateDiagram-v2
    ...
    APPROVED_FOR_DEFENSE --> FINAL_SUBMITTED: Adviser Marks Final Submission
    FINAL_SUBMITTED --> ARCHIVED: Coordinator Archives
    ARCHIVED --> [*]
```
...
*   **Final Submission**: Only Advisers can transition from `APPROVED_FOR_DEFENSE` to `FINAL_SUBMITTED`.
